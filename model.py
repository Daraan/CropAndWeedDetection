...
class CropDataset(torchvision.datasets.VisionDataset):
    ...
    def __init__(self, root=DATA_PATH, *,dataset, 
                 ...
                 seed=None): # should be set in the transformations!):
        """
        From documentation:
        ...
        self.downscale_transform = A.Compose([A.SmallestMaxSize(max(image_size)),
        ...
        if _init_files:
            ...
            if stage == "train":
                ...
                bbox_dir  = os.path.join(self.bbox_root, 'bboxes', dataset+"Eval") # allows more lenient preidctions for validation
    ...
    def cache_images(self):
        ...
        print("Allocated array of size", image_stack.shape, format(image_stack.nbytes / 1024 / 1024 / 1024, ".4f"), "GB")
        image_stack[0] = image
        try:
    ...
    def _format_transformed(self, transformed, index):            
        ...
        if len(bbox) > 0:
            ...
            if BBOX_PARAMS.format == "pascal_voc":
                boxes = torchvision.ops.box_convert(
                    torch.as_tensor(bbox, dtype=torch.float32), "xyxy", "cxcywh" #  if not self.half_precision else torch.float16
                )
        ...
        return (image, 
                ...
                torch.as_tensor(self.image_size)) # height x width (array format)
...
class CropAndWeedDataModule(pl.LightningDataModule):
    ...
    def __init__(self, dataset, data_dir, *, seed=BACKUP_SEED, eval_batch_scale=2,
                 batch_size,
                 num_workers, 
                 image_size,
                 train_transform,
        ...
        if check_exists:
            ...
            if isinstance(self.data_dir, dict):
                ...
                if os.path.exists(self.data_dir["bboxes"]) and os.path.exists(self.data_dir["images"]):
                    print("Data paths already exists. Skipping download and mask generation. No integrity check was performed.")
                    return
                ...
                if self.data_dir["bboxes"] != self.data_dir["images"]:
                    raise FileNotFoundError("Image and BBox path differ but one does not exist. Please specify one path only to download files or check the passed paths", self.data_dir["bboxes"], self.data_dir["images"])
    ...
    def setup(self, stage : str="fit"):
        ...
        if self.indices is None:
            ...
            # Perform train_val_test split and get indices
            self.indices = utils.train_val_test_split(self.all_images, self.seed or BACKUP_SEED, self.train_val_test_ratio) 
...
class YOLO_PL(pl.LightningModule):
    ...
    def on_fit_start(self):
        ...
        print("Fitting", self.settings["model"]["name"], "on", self.settings["dataset"]["name"], "with", self.settings["optimizer"]["name"],end="\n\n")
    ...
    def _log_images(self, images, labels, image_id, predictions):
        ...
        figures = []
        IDX=slice(4)
        pred = self.model.postprocess(predictions, conf_thres=0.00, 
                                                  max_detections=30, 
                                                  multiple_labels_per_box=False
                                                  )
        ...
        bbox_files = [os.path.split(f)[1] for f in self.trainer.val_dataloaders.dataset.bboxes[image_id[IDX].cpu()]] # filenames to to true bounding boxes
        plots = utils.visualize_bboxes(DATASETS[self.settings["dataset"]["name"]], 
                              ...
                              font_scale=0.55,
                              save_to_disk=False,
                              crop_borders=240 if self.settings["dataset"]["image_size"][0] > 1200 else 100, # certically to read original format
                              target_width=640,
                              pbar=False)
        stack = torch.tensor(np.array(plots)) # turn into array first for speed up
        # Turn Batch x HxWxChannels into -> B x C x H x W
        grid =  torchvision.utils.make_grid(stack.permute(0,3,1,2), nrow=2, )
        # Turn into HxWxC
        self.logger.experiment.add_image('val_bboxes', grid.permute(1,2,0), self.current_epoch, 
                                    dataformats='HWC')
    ...
    def configure_optimizers(self):
        ...
        if self.settings['optimizer']['name'] == "SGD":
            ...
            optimizer = DeepSpeedCPUAdam(param_groups["other_params"], 
                                        lr=self.settings["optimizer"]["learning_rate"],
                                        betas=(0.937, 0.999),
                                        weight_decay=self.settings["optimizer"]["weight_decay"])
            optimizer.add_param_group(
            ...
            optimizer = FusedAdam(param_groups["other_params"], 
                                    lr=self.settings["optimizer"]["learning_rate"], 
                                    betas=(0.937, 0.999), 
                                    adam_w_mode=True)
            optimizer.add_param_group(
