...
def train_val_test_split(all_images, seed, train_val_test_ratio):
    ...
    shuffled = geny.choice(len(all_images), len(all_images), replace=False) # not really necessary but from an older implementation
    train_val, test = train_test_split(shuffled, test_size=train_val_test_ratio[-1], random_state=seed)
    train, val = train_test_split(train_val, train_size=(train_val_test_ratio[0] / (train_val_test_ratio[0]+train_val_test_ratio[1])), random_state=seed)
...
def check_worker_count(requested_workers) -> int:
    ...
    if max_num_worker_suggest is None:
        warnings.warn("DataLoader is not able to compute a suggested max number of worker in current system.")
        return requested_workers

    if requested_workers > max_num_worker_suggest:
        warnings.warn(("This DataLoader's requested amount of {} worker processes is larger than the suggested amount of {} "
...
def plot_predictions(model,
    ...
    """
    Note this function is here only to demonstrate how the below function works
    ...
    utils.visualize_bboxes(cnw_dataset, 
                          images, 
                          bbox_files, 
                          predictions=preds, 
...
def make_label_box(image, bbox, text, color=(255,0,0),
                    thickness=2, 
                    font_scale=0.5, 
                    font=cv2.FONT_HERSHEY_SIMPLEX,
                    font_thickness=2):
    ...
    for bboxes_file, image, pred in zip(iterator, images, predictions):
        ...
        with open(os.path.join(bboxes_dir, bboxes_file), 'r') as csv_file:
            ...
            for row in csv_reader:
                ...
                if plot_true_bboxes:
                    ...
                    make_label_box(trueimage, (int(row['left']), int(row['top']),
        ...
        if pred is not None:
            ...
            for box, score, label_id in zip(boxes, scores, label_ids):
                ...
                name += f" {score:.1%}"
                make_label_box(image, box, name, thickness=2, color=color, font_scale=font_scale, font_thickness=FONT_THICKNESS)
        ...
        # Write image path
        cv2.putText(image, target, (10, 25), font, font_scale*1.3, (255, 255, 255),
        ...
        if mask_root:
            ...
            label_layer = cv2.resize(ids2colors(cv2.imread(label_ids_path, 0), cnw_dataset), target_size,
