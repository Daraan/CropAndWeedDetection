...
def parse_arguments():
    ...
    parser.add_argument(
        "--settings_dir", type=str, default="./experiments", help="path to the location of the setting.json files"
    )
    parser.add_argument("--ckpt_path", required=False, help="Optionally load a checkpoint.")
    parser.add_argument("--no_log", required=False, default=True, help="Do not create checkpoint and log files.")
...
def make_augmentations(settings):
    ...
    augmentation_pipeline = A.Compose(
        [
            ...
            *(
                (A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), max_pixel_value=5),)
                if settings["dataset"]["normalize_images"]
                else ()
            ),  # normalizing is bad for YOLO
            # To Tensort
            ToTensorV2(),
        ],
...
def strict_on_validation_epoch_end(self):
    ...
    for mAP, prefix in zip([self.mAP, False], ["", "val_"]):
        ...
        if mAP is False:
            ...
            strict_valmAP = MeanAveragePrecision(box_format="cxcywh", iou_type="bbox", class_metrics=True)
...
def train(settings=None, logname=None):
    ...
    pl_model.to("cpu")
    pl_model.mAP = MeanAveragePrecision(box_format="cxcywh", iou_type="bbox", class_metrics=True)

    pl_model.strict_valmAP = MeanAveragePrecision(box_format="cxcywh", iou_type="bbox", class_metrics=True)
